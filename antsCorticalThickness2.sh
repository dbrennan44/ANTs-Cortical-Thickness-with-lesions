...
#add anat path
ANAT_PATH="$(dirname ${ANATOMICAL_IMAGES[0]})" #added DB
#
        ...
        if [[ ! -f ${SEGMENTATION_WARP} ]];
          ...
          if [[ -f "${ANAT_PATH}/lesion_warped.nii" ]]; then
            ...
            if [[ ${RUN_QUICK} -ne 0 ]];
              ...
              basecall="${basecall} -x [ ${segLESION_EXCLUSION_MASK} ]" # here we can supply a lesion-respecting brain mask
            ...
            for img in ${BRAIN_EXTRACTION_MASK} ${EXTRACTED_SEGMENTATION_BRAIN} ${segLESION_EXCLUSION_MASK};
              do
...
if [[ ! -s ${OUTPUT_PREFIX}ACTStage3Complete.txt ]] && \
  ...
  if [[ ${ACT_STAGE} -eq 0 ]] || [[ ${ACT_STAGE} -eq 3 ]] ; then # BAStages seg
    ...
    # lesion mask not being passed here
    echo "testing testing seg lesion mask is ${segLESION_EXCLUSION_MASK} "
    echo "testing testing seg lesion mask is ${segLESION_EXCLUSION_MASK} "
    echo "testing testing seg lesion mask is ${segLESION_EXCLUSION_MASK} "
    ...
    for (( j = 0; j < ${#ANATOMICAL_IMAGES[@]}; j++ ))
      do
        ATROPOS_ANATOMICAL_IMAGES_COMMAND_LINE="${ATROPOS_ANATOMICAL_IMAGES_COMMAND_LINE} -a ${OUTPUT_PREFIX}BrainSegmentation${j}N4.${OUTPUT_SUFFIX}"; #perhaps error here?
      done
...
if [[ -f ${REGISTRATION_TEMPLATE} ]] && [[ ! -f $REGISTRATION_LOG_JACOBIAN ]];
  then
    ...
    basecall=''
    if [[ ${RUN_QUICK} -ne 0 ]]; # hmm what about quick registration?
      then
      ...
      else #SEEMS THAT THERE SHOULD BE LESION MASK HERE
